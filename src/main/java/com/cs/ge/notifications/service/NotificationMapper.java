...
public abstract class NotificationMapper {
    ...
    protected Map<String, Object> map(final Notification notification, final Recipient to, final Channel channel) {
        try {
            ...
            Map<String, List<Object>> params = Maps.newHashMap(notification.getParams());
            ...
            if (message != null) {
                String messageAsString = messageAsString = message.toString();
            ...
            if (!Strings.isNullOrEmpty(notification.getTemplate())) {
                ...
                messageToSend = NotificationMapper.processTemplate(params, notificationTemplate.getContent(), channel);
            } else {
                messageToSend = messageToSend.replaceAll(Pattern.quote("{{"), Matcher.quoteReplacement("${"))
                        .replaceAll(Pattern.quote("}}"), Matcher.quoteReplacement("}"));
                messageToSend = NotificationMapper.processTemplate(params, messageToSend, channel);
            }
    ...
    protected static NotificationStatus getNotificationStatus(final Notification notification, final String userId, final Channel channel, final String providerId, final String status) {
    ...
    protected static String processTemplate(final Map<String, List<Object>> model, final String template, final Channel channel) {
        ...
        final String[] parsedTemplate = {NotificationMapper.processTemplateWithValues(oneItemMap, template)};
    ...
    private static String processTemplateWithValues(final Map<String, Object> model, final String template) {
