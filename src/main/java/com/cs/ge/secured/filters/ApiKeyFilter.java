...
public class ApiKeyFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        final UUID providedKey = UUID.fromString(serviceKey);
        final UUID providedId = UUID.fromString(serviceId);

        if (SecurityContextHolder.getContext().getAuthentication() == null) {
            ApiKeyAuthenticationManager apiKeyAuthenticationManager = new ApiKeyAuthenticationManager(this.profileService);
            ApiKeyAuthentication authentication = new ApiKeyAuthentication(providedKey, providedId);
